...
import { Mail, Lock, Sparkles, ArrowRight, ShieldCheck, AlertCircle, Loader2 } from 'lucide-react';
...
export const LoginPage: React.FC<LoginPageProps> = ({ onNavigate, onSuccess }) => {
  const { signInWithGoogle, signInWithEmail, continueAsGuest, isMockMode } = useAuth();
      ...
      <div className="w-full max-w-md bg-white dark:bg-[#201A17] rounded-3xl p-6 sm:p-8 border border-[#E86F51]/15 shadow-xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-[#716761] dark:text-[#A89E97]">
            Tiếp tục hành trình chinh phục tiếng Trung giao tiếp cùng Lina.
          </p>
        </div>
        ...
        {isMockMode && (
          <div className="p-3 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900/50 flex items-start gap-2.5 text-xs text-amber-800 dark:text-amber-300">
            ...
            <div>
              <p className="font-bold">Chế độ Demo / Mock Authentication</p>
              <p className="text-[11px] opacity-90 mt-0.5">
                Bạn có thể đăng nhập ngay với bất kỳ email/mật khẩu hoặc nhấn Google Login để thử nghiệm.
              </p>
            </div>
          </div>
        )}
        ...
        {/* Error Alert */}
        {error && (
          <div className="p-3.5 rounded-2xl bg-red-50 dark:bg-red-950/50 border border-red-200 dark:border-red-900/50 flex items-start gap-2.5 text-xs text-red-700 dark:text-red-300 animate-fade-in">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button
                ...
                className="text-xs text-[#E86F51] hover:underline font-medium cursor-pointer"
              >
                Quên mật khẩu?
              </button>
            </div>
        ...
        <div className="pt-2 text-center space-y-3">
          ...
          <div className="border-t border-[#EDE4DB] dark:border-[#382E27] pt-3">
            ...
            >
              Tiếp tục học ở chế độ Khách (Guest Mode)
            </button>
          </div>
        </div>
      </div>
